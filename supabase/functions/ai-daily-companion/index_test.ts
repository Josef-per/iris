...
test("servidor monta markdown a partir de campos simples", () => {
  assert.match(
    source,
    /title: \{\s+type: \["string", "null"\],\s+minLength: 3,\s+maxLength: 80,/,
  );
  assert.match(
    source,
    /introduction: \{\s+type: \["string", "null"\],\s+minLength: 20,\s+maxLength: 300,/,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /text: \{\s+type: "string",\s+minLength: 12,\s+maxLength: 360,/,
  );
  assert.match(source, /cleanCompleteSentence\(value\.introduction, 20, 300\)/);
  assert.match(source, /cleanPlainField\(valuePoint\.label, 2, 29\)/);
  assert.match(source, /cleanCompleteSentence\(valuePoint\.text, 12, 360\)/);
